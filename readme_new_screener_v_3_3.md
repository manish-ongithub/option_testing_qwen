...
| Feature | Description |
|---------|-------------|
| **Opstra IV Integration** | Fetches real IV, IV Percentile, and IV Rank from Opstra API |
| **HV Fallback** | Calculates Historical Volatility when Opstra unavailable |
| **LTP Fallback** | Works after market hours using Last Traded Price |
| **Market Regime Detection** | Adjusts behavior based on VIX levels |
| **Tiered Stock Classification** | Different thresholds for Tier 1, 2, and 3 stocks |
| **Technical Indicators** | RSI and EMA-based trend detection |
| **Days to Expiry** | Shows remaining days until expiration |
| **Lot Size & Total Cost** | Calculates capital required per trade |
| **Breakeven Price** | Shows price needed to profit |
| **Distance from Spot** | Shows strike distance as percentage |
| **IV Source Tracking** | Shows where IV data came from (Opstra/HV/Default) |
...
| File | Description |
|------|-------------|
| `new_screener_v3.3.py` | **Latest** - With Opstra IV + HV fallback (recommended) |
| `new_screener_v3.2.py` | Previous version with hardcoded IV defaults |
| `new_screener_v3.1.py` | Basic version without enhanced fields |
| `diagnostic_screener.py` | Debugging tool to analyze filtering |
...
## IV Data Sources (v3.3)

The screener uses a **fallback chain** for IV data:

### 1. Opstra API (Best - When Configured)

```
Source: https://opstra.definedge.com/api/ivcharts/{SYMBOL}
Data: Real IV, IV Percentile, IV Rank (252-day window)
Reliability: ✅ High - Actual implied volatility from options market
Requirement: Valid session cookies (JSESSIONID, DSESSIONID)
```

### 2. Historical Volatility Calculation (Fallback)

```
Source: Calculated from yfinance price data
Data: 30-day rolling HV, annualized
Reliability: ⚠️ Medium - Approximation, not true IV
Note: HV ≠ IV, but provides relative comparison
```

### 3. Default Values (Last Resort)

```
Data: IV=0, IVP=50%, IVR=50%
Reliability: ❌ Low - No actual data, assumes middle of range
Warning: Displayed in output when used
```

### IV Source Indicator in Output

The output shows where IV data came from:

| IV_SRC | Meaning | Reliability |
|--------|---------|-------------|
| `OPS` | Opstra API | ✅ High |
| `HV_` | Historical Volatility | ⚠️ Medium |
| `DEF` | Default values | ❌ Low |

### Configuring Opstra (Recommended)

For accurate IV data:

1. **Login to Opstra**: Go to [opstra.definedge.com](https://opstra.definedge.com)
2. **Get Cookies**: Press F12 → Application → Cookies → Copy values
3. **Set in Script**: 
...
# Option 2: Call function at runtime
set_opstra_cookies('your_jsessionid', 'your_dsessionid')
```

### Why IV Matters for Options Buyers

| IV Percentile | Meaning | Action |
|---------------|---------|--------|
| **< 30%** | IV is LOW (cheap premiums) | ✅ Good time to BUY options |
| **30-70%** | IV is NORMAL | ⚠️ Standard conditions |
| **> 70%** | IV is HIGH (expensive) | ❌ Avoid buying, premiums inflated |
| **> 85%** | IV is VERY HIGH | 🚫 Script skips these (if Opstra configured) |
...
### v3.3 (Current)
- **Added**: Opstra API integration for real IV data
- **Added**: Historical Volatility (HV) calculation as fallback
- **Added**: IV source tracking (`opstra`, `hv_calculated`, `default`)
- **Added**: IV-based filtering (skips stocks with IVP > 85% when Opstra configured)
- **Added**: IV caching (5-minute cache to reduce API calls)
- **Added**: `IV_SRC` column in output showing data source
- **Added**: Warning when using default IV values
- **Added**: `set_opstra_cookies()` function for easy configuration
- **Fixed**: IV data now actually fetched instead of hardcoded defaults
...
PRE-TRADE CHECKLIST
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
□ Do I have a CLEAR view? (Bearish/Bullish/Neutral)
   Your answer: ________________

□ Why am I bearish/bullish?
   □ Technical (chart breakdown/breakout)
   □ Fundamental (news, earnings, events)
   □ Global cues (US markets, crude, etc.)
   
□ Is ₹4,439 an amount I can lose 100%?
   □ Yes → Proceed
   □ No → Reduce size or skip
   
□ What's my exit plan?
   □ Stop Loss: Exit if premium drops to ₹___
   □ Target: Exit if premium rises to ₹___
   □ Time Stop: Exit by date ___________

□ Have I verified on my broker?
   □ Check live bid-ask spread
   □ Confirm lot size
   □ Check margin/premium required
